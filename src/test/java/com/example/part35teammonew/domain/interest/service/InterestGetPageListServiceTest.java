...
public class InterestGetPageListServiceTest {
  ...
  void listInterests_search_success() {
    ...
    PageImpl<Interest> page = new PageImpl<>(interestList,
        PageRequest.of(0, 10, Sort.by("name").ascending()),
        interestList.size());
    given(interestRepository.searchByNameOrKeyword(eq(search), any(Pageable.class))).willReturn(
        page);

    //Mongo db 첫번째에 user id 포함
    InterestUserList userInterestList = InterestUserList.setUpNewInterestUserList(
        interest1.getId());
    userInterestList.addUser(userId);

    given(userListService.checkUserSubscribe(interest1.getId(), userId)).willReturn(true);
    given(userListService.countSubscribedUser(interest1.getId())).willReturn(1L);
    given(userListService.checkUserSubscribe(interest2.getId(), userId)).willReturn(false);
    given(userListService.countSubscribedUser(interest2.getId())).willReturn(0L);
  ...
  void listInterests_cursorCount_asc() {
    ...
    List<Interest> pageList = List.of(interest3, interest1, interest2);
    ...
    given(userListService.checkUserSubscribe(interest1.getId(), userId)).willReturn(false);
    given(userListService.countSubscribedUser(interest1.getId())).willReturn(7L);
    given(userListService.checkUserSubscribe(interest2.getId(), userId)).willReturn(false);
    given(userListService.countSubscribedUser(interest2.getId())).willReturn(9L);
    given(userListService.checkUserSubscribe(interest3.getId(), userId)).willReturn(false);
    given(userListService.countSubscribedUser(interest3.getId())).willReturn(5L);
    ...
    assertThat(result.content()).extracting(InterestDto::getSubscriberCount)
        .containsExactly(5L, 7L, 9L);
