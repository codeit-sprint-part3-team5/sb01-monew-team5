...
public class InterestServiceImpl implements InterestService {
  ...
  public List<Pair<String, UUID>> getInterestList() {
    ...
    return interestList.stream().flatMap(interest -> {
      ...
      List<String> keywords = Stream.of(interest.getKeywords().split(",")).map(String::trim)
          .toList();
      log.info("관심사 전체 리스트 반환 완료");
      // 이름  키워드를 하나의 스트림으로
      return Stream.concat(Stream.of(interest.getName().strip()), keywords.stream())
  ...
  public PageResponse<InterestDto> listInterests(InterestPageRequest req) {
    ...
    Sort sort = Sort.by(new Sort.Order(direction, orderBy), new Sort.Order(direction, "createdAt"));
    ...
    Pageable pageable = PageRequest.of(0, limit + 10, sort); // 오버페치(+10)로 여유 확보
    ...
    if (isSearching) {
      ...
      log.debug("커서 기반 페이지네이션 적용 - orderBy = {}, rawCursor = {}, normalizedCursor = {}, after = {}",
          orderBy, rawCursor, normalizedCursor, after);

      if (orderBy.equalsIgnoreCase("name")) {
        List<Interest> page =
            direction == Sort.Direction.ASC ? interestRepository.findByNameAfter(null, after,
                pageable) : interestRepository.findByNameBefore(null, after, pageable);

        // 커서 이후 항목 필터링
        interests = page.stream().filter(i -> {
          if (normalizedCursor == null) {
            return true;
          }
          String normalizedName = i.getName().replaceAll("\\s+", "");
          return direction == Sort.Direction.ASC ? normalizedName.compareTo(normalizedCursor) > 0
        ...
        Long countCursor =
            (rawCursor == null || rawCursor.isBlank()) ? null : Long.parseLong(rawCursor);
        interests = direction == Sort.Direction.ASC ? interestRepository.findBySubscriberCountAfter(
            countCursor, after, pageable)
      ...
      if (hasNext && !interests.isEmpty()) {
        ...
        if (orderBy.equalsIgnoreCase("subscriberCount")) {
          ...
          nextCursor = last.getName(); // 원본 이름 기준으로 커서 반환
        }
      }
    }

    List<InterestDto> content = interests.stream()
        .map(i -> mapToDtoWithSubscription(i, req.userId())).toList();

    log.info("관심사 조회 완료 - 조회 건수 = {}, hasNext = {}, nextCursor = {}", content.size(), hasNext,
        nextCursor);
    return new PageResponse<>(content, nextCursor, nextAfter, content.size(), totalElements,
        hasNext);
  }
  ...
  private InterestDto mapToDtoWithSubscription(Interest entity, UUID userId) {
    ...
    List<String> keywordList =
        entity.getKeywords() != null ? Arrays.stream(entity.getKeywords().split(","))
            .map(String::strip).toList() : List.of();

    return InterestDto.builder().id(entity.getId()).name(entity.getName()).keywords(keywordList)
        .subscriberCount(subscriberCount).subscribedByMe(subscribedByMe).build();
  }
